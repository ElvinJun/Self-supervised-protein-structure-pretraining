...
# Load config file
default_param_file = '/home/caiyi/workspace/parameters.json'
...
if finetune:
    ...
    model = MLP(input_dim=input_shape[1], stacked_dim=input_shape[0]).to(device)
...
exec('optimizer = torch.optim.%s(model.parameters(), lr=learning_rate)' % p['optimizer'])
...
if args.cv:
    shutil.copy('/home/caiyi/workspace/cv.sh', os.path.join(save_path, 'cv.sh'))
