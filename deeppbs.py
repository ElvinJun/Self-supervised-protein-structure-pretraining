...
class KnnBatch(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for filename in filenames:
            ...
            array = np.load(os.path.join(self.input_path, f'{filename}.npy'))
            knn.append(array.reshape(-1, array.shape[1] * array.shape[2]))
            lengths.append(np.shape(knn[-1])[0])
            seq = list(self.seq_dict[pdb_name])
            seqs.append([rep_utils.AA_NUM[aa] for aa in seq])
        seqs = [torch.tensor(seq) for seq in seqs]
        #print("HHH", seqs[0].shape)
        knn = np.concatenate(knn)
        return knn, seqs, filenames, lengths
...
class KNN2tor(Dataset):
    ...
    def __init__(self, path, list_path):
        ...
        if list_path == None:
            self.file_list = os.listdir(path['knn_75'])
        else:
...
class KNN2TorNoStruct(Dataset):
    ...
    def __getitem__(self, idx):
        filename = self.file_list[idx]
        knn = np.load(os.path.join(self.input_path, filename))
        knn = knn[:, 0, -3:]
        return knn, filename
...
class BatchLSTM(nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        for length in lengths:
            next_ = last + length
            #print(x.shape, x[last:next_].shape, last, next_)
...
class DeepPBS(nn.Module):
    def __init__(self, blocks, dims):
        ...
        elif blocks['predict'] == '2HidRelu':
            self.predicting_blocks = MLP2HiddenReLU(input_dim=global_dim, stacked_dim=222).cuda()
    ...
    # MODIFIED
    # def load_model(self, checkpoint, blocks=['local', 'global', 'predict']):
