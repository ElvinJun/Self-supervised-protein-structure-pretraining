...
model_path = '/home/joseph/KNN/outputs/self_20201016_4_sota_cm_knn150_49600/111_Linear.pth'
output_path = '/share/seqtonpy/rocklin/knn_self_512_full/self_20201018_1_knn150_pre__20201016_4_sota_cm_knn150_49600'
...
class Semilabel(nn.Module):
    ...
    def __init__(self, input_dim=args.input_dim, hidden_dim=256, feature_dim=256, output_dim=20):
        ...
        self.hidden3 = nn.Linear(hidden_dim * 2, hidden_dim * 3)
        self.ln4 = nn.LayerNorm(hidden_dim * 3)

        self.hidden4 = nn.Linear(hidden_dim * 3, hidden_dim * 2)
        self.ln5 = nn.LayerNorm(hidden_dim * 2)
    ...
    def forward(self, arrays):
        ...
        # hidden_states = hidden_states.unsqueeze(1)
        # hidden_states, _ = self.lstm(hidden_states)
        hidden_states = swish_fn(self.ln3(self.hidden2(hidden_states)))
        hidden_states = self.hidden3(hidden_states)
        # hidden_states = swish_fn(self.ln4(self.hidden3(hidden_states)))
        # hidden_states = swish_fn(self.ln5(self.hidden4(hidden_states)))
        # hidden_states = swish_fn(self.ln6(self.hidden(hidden_states)))
        # output = self.predict(hidden_states)
        # # output = F.softmax(output, dim=-1)
        # output = self.dropout(output)
