...
network_dict = {'our': 'MLP', 'tape': 'ValuePredictionHead', 'relu': 'MLPReLU', 'relu_wn': 'MLPReLUWeightNorm',
                ...
                'MLPAttentionFixedHiddenSigmoid','att_fixed_sig_768':
                'MLPAttentionFixedHiddenSigmoid_768', 'att_fixed_relu': 'MLPAttentionFixedHiddenReLU', 'att_sig_test1':
...
class MLP2FixedHiddenReLU(nn.Module):
    def __init__(self, stacked_dim, input_dim,  hidden_dim=512):
        super().__init__()
        self.fc1 = nn.Linear(input_dim, hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, hidden_dim // 2)
        self.output = nn.Linear(hidden_dim, 1)
...
class MLPAttentionFixedHiddenSigmoid_768(nn.Module):
    def __init__(self, stacked_dim, input_dim=768,  hidden_dim=768):
        super().__init__()
        self.fc0 = nn.Linear(input_dim, 1)
        self.fc1 = nn.Linear(input_dim, hidden_dim)
        self.ln = nn.LayerNorm(hidden_dim)
