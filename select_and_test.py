...
parser.add_argument('-g', '--gpu', help='GPU to use to run prediction on the test set, e.g. 0', default='0')
parser.add_argument('-n', '--number', help='Number of best models to use to test.', default=5)
...
min_loss = sorted(val_loss.keys())[:n]
...
if criterion == 'r':
    ...
    models = [min_loss[i] for i in valid_performs]
else:
